...
test("Alpha watcher classifies configured recent-output paths before timeline substrings", async () => {
  ...
  callback("change", "recent-output.jsonl");
  await new Promise((resolve) => setTimeout(resolve, 10));
  assert.deepEqual(events[0].sourceKinds, ["summary"]);
  watcher.close();
});
...
test("default Alpha watcher keeps configured summary invalidations exact without cwd noise", async () => {
  ...
  const summaryPath = join(summaryRoot, "recent-output.jsonl");
  ...
  summaryRegistration.onEvent("change", "recent-output.jsonl");
  await new Promise((resolve) => setTimeout(resolve, 10));
...
test("Alpha keeps explicit Projects separate and does not associate runtime by cwd", () => {
  ...
  writeFileSync(
    ...
    '{"type":"output_summary","projectId":"one","summary":"one","observedAt":"2026-07-12T10:00:00.000Z"}\n',
  );
  writeFileSync(
...
test("Alpha adapters preserve stale/malformed states and avoid bd list --limit 0 semantics", () => {
  ...
  writeFileSync(
    summaryPath,
    '{"type":"output_summary","summaryId":"old","summary":"visible summary","observedAt":"2026-07-01T00:00:00.000Z"}\nnot-json\n',
  );
...
test("canonical registry validation stays in parity with the distiller and fails closed", () => {
  ...
  const cases = [
    ...
    },
    {
      ...
      build: (value) => {
        value.projects[0].locations.summaries = "recent-output.jsonl";
        return value;
      },
    },
    {
...
test("Alpha retains summary failure records as diagnostics instead of crashing", () => {
  ...
  const path = join(root, "recent-output.jsonl");
  writeFileSync(
    path,
    [
      JSON.stringify({
